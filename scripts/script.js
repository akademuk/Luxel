...
function initDropdowns() {
  ...
  function selectLanguage(code, event) {
    ...
    const options = document.querySelectorAll(
      ".header__promo-language-dropdown-option"
    );
  ...
  document.addEventListener("click", function (event) {
    ...
    const languageDropdowns = document.querySelectorAll(
      ".header__promo-language-dropdown"
    );
    ...
    callFormBurger.addEventListener("submit", function (e) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          if (
            successModal &&
            successModal.classList.contains(stateClasses.active)
          ) {
            closeSuccessModal();
          }
        }, 5000);
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      if (
        ...
        emailMenu &&
        emailMenu.classList.contains(stateClasses.isOpen)
      ) {
        closeEmailMenu(e);
      } else if (
        hiddenMenu &&
        hiddenMenu.classList.contains(stateClasses.isOpen)
      ) {
        closeHiddenMenu(e);
...
function initCatalogMenu() {
  ...
  const submenuContents = document.querySelectorAll(
    ".menu__container-submenus-content"
  );
    ...
    new Swiper(".hero__slider", {
      ...
      pagination: {
        ...
        renderBullet: function (index, className) {
          return (
            '<span class="' +
            className +
            '"><span class="swiper-pagination-bullet-progress"></span></span>'
          );
        },
      },
      breakpoints: {
      ...
      on: {
        ...
        autoplayTimeLeft: function (swiper, time, progress) {
          ...
          const paginationEl =
            typeof swiper.pagination.el === "string"
  ...
  closeButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (menu) {
        ...
        const anyMenuOpen = document.querySelector(
          ".slide-menu.active, #account-menu.active"
        );
        if (!anyMenuOpen) {
...
function initRangeSlider() {
  ...
  class RangeSlider {
    ...
    attachEventListeners() {
      this.priceFrom.addEventListener("input", () =>
        this.handleInputChange("from")
      );
      this.priceTo.addEventListener("input", () =>
        this.handleInputChange("to")
      );

      this.thumbFrom.addEventListener("mousedown", (e) =>
        this.startDrag(e, "from")
      );
      this.thumbTo.addEventListener("mousedown", (e) =>
        this.startDrag(e, "to")
      );

      this.thumbFrom.addEventListener(
        "touchstart",
        (e) => this.startDrag(e, "from"),
        { passive: false }
      );
      this.thumbTo.addEventListener(
        "touchstart",
        (e) => this.startDrag(e, "to"),
        { passive: false }
      );
      ...
      this.thumbFrom.addEventListener("keydown", (e) =>
        this.handleKeyboard(e, "from")
      );
      this.thumbTo.addEventListener("keydown", (e) =>
        this.handleKeyboard(e, "to")
      );
    }
    ...
    handleKeyboard(e, type) {
      const step = 100;
      let value = parseInt(
        type === "from" ? this.priceFrom.value : this.priceTo.value
      );
      ...
      if (type === "from") {
        this.priceFrom.value = Math.max(
          this.min,
          Math.min(value, parseInt(this.priceTo.value) - 10)
        );
      } else {
        this.priceTo.value = Math.min(
          this.max,
          Math.max(value, parseInt(this.priceFrom.value) + 10)
        );
      }
...
function initMobileFilter() {
  ...
  function updateFilterButton() {
    ...
    const filterContent = filterBtn.querySelector(
      ".mobile-filter__content-option"
    );
    if (filterContent) {
...
function initSortPopup() {
  ...
  sortRadios.forEach((radio) => {
    radio.addEventListener("change", () => {
      if (radio.checked && sortOption) {
        ...
        const selectedText =
          selectedLabel.querySelector(".sort-popup__text").textContent;
  ...
  document.addEventListener("click", (e) => {
    if (
      sortPopup.classList.contains("active") &&
...
function initProductAccordion() {
  ...
  items.forEach((item) => {
    ...
    header.addEventListener("click", () => {
      ...
      items.forEach(
        (i) =>
          (i.querySelector(".product-detail__accordion-content").hidden = true)
      );
...
function initAccordionProduct() {
  ...
  function togglePanel(header) {
    ...
    const panel = accordionItem.querySelector(
...
function initNotifyAvailability() {
  ...
  document
    .getElementById("closeNotifyModal")
...
function initSpecificationsModal() {
  ...
  const productCode =
    document.querySelector(".product-detail__code")?.textContent?.trim() ||
    "063-NEL 18W";
...
function initReviewsModal() {
  const reviewsBtns = document.querySelectorAll(
    ".product-detail__reviews-rating-btn"
  );
  ...
  // Закриття модалки
  document
    .getElementById("closeReviewsModal")
    ?.addEventListener("click", closeModal);
...
function initProductGallery() {
  ...
  const productCode =
    document.querySelector(".product-detail__code")?.textContent || "";
...
function initReviewsGallery() {
  ...
  Fancybox.bind('[data-fancybox="galleryReviews"]', {
    ...
    caption: function (fancybox, slide) {
      ...
      if (reviewItem) {
        const author =
          reviewItem.querySelector(".reviews-list__item-autor h3")
            ?.textContent || "";
        const date =
          reviewItem.querySelector(".reviews-list__item-autor time")
            ?.textContent || "";
        ...
        setTimeout(() => {
          ...
          if (container) {
            container.addEventListener("click", (e) => {
              if (
                !e.target.closest(".fancybox__carousel") &&
                ...
                console.log(
                  "🔴 Closing modal gallery - click outside carousel"
                );
                fancybox.close();
              }
            });
        ...
        try {
          ...
          const updateNavigationButtons = () => {
            try {
              ...
              const prevButton = document.querySelector(
                ".fancybox__nav .f-button[data-fancybox-prev]"
              );
              const nextButton = document.querySelector(
                ".fancybox__nav .f-button[data-fancybox-next]"
              );
          ...
          const centerThumb = () => {
            try {
              ...
              const activeThumb = thumbsWrapper.querySelector(
                `.f-thumbs__slide:nth-child(${currentIndex + 1})`
              );
...
function toggleAnswerVisibility() {
  const toggleButtons = document.querySelectorAll(
    ".reviews-list__item-description-answer-btn"
  );

  toggleButtons.forEach((button) => {
    button.addEventListener("click", function () {
      const answerBox = button.closest(
        ".reviews-list__item-description-answer"
      );
      const contentBox = answerBox.querySelector(
        ".reviews-list__item-description-answer--box"
      );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const reviewsItems = document.querySelectorAll(
    ".reviews-list__item, .reviews-list__image"
  );
...
function initShoppingCart() {
  ...
  function updateDecrementButton(productItem) {
    const quantityEl = productItem.querySelector(
      ".shopping-cart-product-item-quantity-number"
    );
  ...
  function updateTotals() {
    ...
    items.forEach((item) => {
      const quantity = parseInt(
        item.querySelector(".shopping-cart-product-item-quantity-number")
          .textContent
      );
    ...
    document.getElementById("total-old").textContent =
      totalOld.toFixed(2) + " ₴";
    ...
    const progress = Math.min(
      100,
      (totalCurrent / FREE_DELIVERY_THRESHOLD) * 100
    );
  ...
  function showDeleteConfirmation(productItem) {
    const productName =
      productItem
        .querySelector(".shopping-cart-product-item-header h2")
        ?.textContent.trim() || "товар";
...
function initReviewModal() {
  ...
  document.addEventListener("click", (e) => {
    const closeBtn = e.target.closest(
      "[data-modal-close], .modal__back, .js-modal-close"
    );
    if (closeBtn) {
  ...
  if (imagePreview) {
    imagePreview.addEventListener("click", function (e) {
      if (e.target.classList.contains("remove-image")) {
        const idx = Array.from(imagePreview.children).indexOf(
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", function (e) {
      ...
      if (!validateLogin(loginInput.value.trim())) {
        const errorSpan =
          loginInput.parentElement.querySelector(".novalid-style");
  ...
  if (backBtn) {
    backBtn.addEventListener("click", function (e) {
      ...
      if (codeInput) {
        codeInput.value = "";
        const errorSpan =
          codeInput.parentElement.querySelector(".novalid-style");
        if (errorSpan) errorSpan.style.display = "none";
  ...
  document.addEventListener("click", function (e) {
    if (accountMenu && accountMenu.classList.contains("active")) {
      const content =
        accountMenu.querySelector(".modal__content") ||
        accountMenu.querySelector(".slide-menu__content");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (backBtn) {
    backBtn.addEventListener("click", function (e) {
      ...
      if (modalContent)
        modalContent.classList.remove("modal__content-account-timer");
      ...
      if (input === phoneInput) {
        input.addEventListener("blur", () => {
          ...
          if (phoneVal.length > 0 && phoneVal.length < 10) {
            const errorSpan =
              input.parentElement.querySelector(".novalid-style");
            if (errorSpan) errorSpan.style.display = "block";
  ...
  if (toggle && emailField) {
    toggle.addEventListener("change", (e) => {
      if (e.target.checked) {
        ...
        if (emailInput) {
          const errorSpan =
            emailInput.parentElement.querySelector(".novalid-style");
          if (errorSpan) errorSpan.style.display = "none";
  ...
  if (registerForm) {
    registerForm.addEventListener("submit", (e) => {
      ...
      if (phoneVal.length < 10) {
        const errorSpan =
          phoneInput.parentElement.querySelector(".novalid-style");
        if (errorSpan) errorSpan.style.display = "block";
      ...
      if (modalContent)
        modalContent.classList.add("modal__content-account-timer");
...
function initAddressTab() {
  ...
  function setSelectValue(selectId, value) {
    ...
    const option = select.querySelector(
      `.select-items div[data-value="${value}"]`
    );
    ...
    if (selected) {
      if (option) {
        ...
        if (!value) {
          if (selectId === "select-country")
            selected.innerHTML = "Оберіть країну";
          if (selectId === "select-region")
            selected.innerHTML = "Оберіть область";
          if (selectId === "select-city") selected.innerHTML = "Оберіть місто";
          if (selectId === "select-delivery")
            selected.innerHTML = "Оберіть спосіб доставки";
          if (selectId === "select-street")
            selected.innerHTML = "Оберіть вулицю";
          if (selectId === "select-branch")
            selected.innerHTML = "Оберіть відділення";
        }
      }
    }
  ...
  function populateForm(data) {
    ...
    if (data.delivery === "address" || data.delivery === "courier") {
      ...
      if (form.querySelector('input[name="house"]'))
        form.querySelector('input[name="house"]').value = data.house;
      if (form.querySelector('input[name="apartment"]'))
        form.querySelector('input[name="apartment"]').value = data.apartment;
      if (form.querySelector('input[name="zip"]'))
        form.querySelector('input[name="zip"]').value = data.zip;
  ...
  // Закриття модалки
  const closeBtns = addressModal
    ? addressModal.querySelectorAll(".acc-modal__close")
    : [];
  closeBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
  ...
  // Custom Select Logic
  const selects = addressModal
    ? addressModal.querySelectorAll(".custom-select")
    : [];
      ...
      options.forEach((option) => {
        option.addEventListener("click", (e) => {
          ...
          if (select.id === "select-country") {
            ...
            const addressDetails = document.getElementById("address-details");
            if (addressDetails) {
              const inputsToEnable = addressDetails.querySelectorAll(
                'input[name="house"], input[name="apartment"], input[name="zip"]'
              );
  ...
  function handleDeliveryChange(value) {
    ...
    if (value === "address" || value === "courier") {
      if (addressDetails) {
        ...
        const inputsToDisable = addressDetails.querySelectorAll(
          'input[name="house"], input[name="apartment"], input[name="zip"]'
        );
    ...
    if (isValid) {
      if (delivery === "address" || delivery === "courier") {
        ...
        const apartment = form
          .querySelector('input[name="apartment"]')
          .value.trim();
  ...
  if (!window.addressMenuListenerAdded) {
    document.addEventListener("click", (e) => {
      if (
        !e.target.closest(".address-card__menu") &&
        !e.target.closest(".address-card__menu-btn")
      ) {
        document
          .querySelectorAll(".address-card__menu.active")
  ...
  if (saveBtn) {
    saveBtn.addEventListener("click", (e) => {
      ...
      const citySelect = document.getElementById("select-city");
      if (citySelect) {
        const selectedOption = citySelect.querySelector(
          `.select-items div[data-value="${cityVal}"]`
        );
      ...
      const deliverySelect = document.getElementById("select-delivery");
      if (deliverySelect) {
        const selectedOption = deliverySelect.querySelector(
          `.select-items div[data-value="${delivery}"]`
        );
      ...
      if (delivery === "address" || delivery === "courier") {
        ...
        const streetSelect = document.getElementById("select-street");
        if (streetSelect) {
          const selectedOption = streetSelect.querySelector(
            `.select-items div[data-value="${streetVal}"]`
          );
        ...
        const branchSelect = document.getElementById("select-branch");
        if (branchSelect) {
          const selectedOption = branchSelect.querySelector(
            `.select-items div[data-value="${branchVal}"]`
          );
